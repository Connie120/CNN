...
module acl_scfifo_wrapped #(  
  ...
  parameter connect_clr_to_scfifo   = 0              // If true the pass in the aclr and sclr signal to scfifo ,aclr and scrl will now also be used for register from ecc decoder signals 
) 
(
  ...
  logic aclr_int;                                // aclr signal to be sent to altera_syncram
  ...
  generate
   if(connect_clr_to_scfifo) begin
     assign aclr_int = aclr;
     assign sclr_int = sclr; 
   end else begin
     assign aclr_int = 1'b0;
