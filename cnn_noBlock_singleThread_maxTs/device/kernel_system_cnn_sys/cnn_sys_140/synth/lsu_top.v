...
begin
   ...
   // when HYPER_PIPELINE=1, use the new S10 optimized hld_fifo for best FMAX and to support the Reset protocol
   if (HYPER_PIPELINE) begin
      logic nop_fifo_empty_n;
      ...
      hld_fifo #(
          ...
          .STYLE("ll"),                                // always use a low latecny (register based) FIFO when HYPER_PIPELINE is enabled
   ...
   end else if(KERNEL_SIDE_MEM_LATENCY <= 64)
   begin
      acl_ll_fifo #(
         .WIDTH(1),
         .DEPTH(KERNEL_SIDE_MEM_LATENCY+1),
